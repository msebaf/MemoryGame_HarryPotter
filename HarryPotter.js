...
boton.addEventListener("click", async event =>{
   ...
   document.getElementById("found").innerHTML= "Found: "+coincidencias;
    ...
    colocarCartas(await personajes);
    let loading = document.createElement("p");
    loading.setAttribute("id", "cargando")
    loading.innerHTML="LOADING..."
    let marco = document.getElementById("marco-juego")
    marco.appendChild(loading)
    setTimeout(() => {
        loading.innerHTML="BEGIN"
    },2000)
    setTimeout(() => {
        marco.removeChild(loading)
    },3000)
    actualizarTiempo();
  ...
  async function elegir10(datos){
    ...
    async function colocarCartas(pers){
        ...
        for (let i = 0; i < 20; i++) {
            ...
            carta.addEventListener("click", async event=> {
                flip.play()
                
                var card = event.target;
                if (card.getAttribute("src") == "reverso_carta.jpg"){
                    await card.setAttribute("src", final[card.getAttribute("id")].image)
                    cantCartasGiradas += 1 
